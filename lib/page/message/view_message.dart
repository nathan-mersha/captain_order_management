              ...
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.connectionState == ConnectionState.done) {
                  ...
                  _MessageDataSource _messageDataSourceVal =
                      _MessageDataSource(context, messages, () {
                    setState(() {
                      // updating table here.
                    });
                    ...
                    columns: [
                      DataColumn(
                        ...
                        onSort: (columnIndex, ascending) {
                          return _sort<String>(
                              (d) => d.recipient, columnIndex, ascending);
                        },
                      ),
                      DataColumn(
                        label: Text("message"),
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (d) => d.body, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: Text("Date"),
                        onSort: (columnIndex, ascending) => _sort<DateTime>(
                            (d) => d.firstModified, columnIndex, ascending),
                      ),
                    ],
...
class _MessageDataSource extends DataTableSource {
  ...
  final GlobalKey<CreateMessageViewState> createMessageKey;
  _MessageDataSource(
      this.context, this.messages, this.updateTable, this.createMessageKey);
  ...
  DataRow getRow(int index) {
    ...
    return DataRow.byIndex(
      ...
      cells: [
        ...
        DataCell(Text(DateFormat.yMMMd().format(message.firstModified),
            style: TextStyle(color: Colors.black54))),
      ],
    );
  }
